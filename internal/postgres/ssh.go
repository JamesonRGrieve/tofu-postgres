...
//
// SSH transport for PostgreSQL host management. PostgreSQL exposes no REST API
// for its installed state (apt packages), its config files (postgresql.conf /
// pg_hba.conf), its service (systemd / pg_ctlcluster), or HA bring-up
// (streaming / repmgr / Patroni) — all of it is driven over SSH/CLI on the DB
// host. The provider's typed resources drive those commands through this
// transport.
//
// Like the sibling tofu-opnsense / tofu-proxmox providers, we invoke the system
// `ssh` binary via os/exec rather than an in-process SSH library — this keeps
// go.mod minimal (no golang.org/x/crypto/ssh) and reuses the lab's existing SSH
// machinery (OpenBao-signed certs / agent / ssh_config). A private key is only
// ever materialized when key_pem is supplied (written to a temp 0600 file per
// call, removed after); key_file and ssh_config paths never touch the material.
// Auth is key/cert ONLY — never sshpass, never a password.
package postgres
...
// SSHClient runs remote commands on a PostgreSQL host over SSH. Safe for
// concurrent use; each call spawns its own ssh process. nil when the provider
// was configured without an ssh_host.
...
// User returns the SSH login user (so a resource managing access can keep the
// provider's own transport from being locked out).
...
const (
	defaultSSHTimeout = 45 * time.Second
	maxSSHAttempts    = 4
...
func (e *SSHError) Error() string {
	return fmt.Sprintf("postgres ssh %q: exit %d: %s", e.Cmd, e.ExitCode, strings.TrimSpace(e.Stderr))
}
...
// Run executes a remote command (handed to the host's shell), retrying transient
// connection resets, and returns stdout. stdin (may be nil) is piped to the
// command — used to feed a config blob to `tee`/`cat` without shell-quoting it.
func (c *SSHClient) Run(remote string, stdin []byte) ([]byte, error) {
	...
	for attempt := 1; attempt <= maxSSHAttempts; attempt++ {
		...
		time.Sleep(time.Duration(attempt) * transientBackoff)
	}
	return out, err
}
...
func (c *SSHClient) runOnce(remote string, stdin []byte) ([]byte, error) {
	...
	args := []string{
		...
		// NO ControlMaster: the lab jump-host relay is a single-shot busybox
		// `nc -e` pipe that one persistent multiplexed connection breaks.
	...
	if kerr != nil {
		return nil, fmt.Errorf("postgres: materialize ssh identity: %w", kerr)
	}
	defer cleanup()
	if keyPath != "" {
	...
	if err := cmd.Run(); err != nil {
		...
		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("postgres ssh %q: timed out after %s", remote, c.timeout)
		}
...
func (c *SSHClient) identityFile() (path string, cleanup func(), err error) {
	...
	f, err := os.CreateTemp("", "postgres-key-*")
	if err != nil {
		return "", noop, err
	}
	name := f.Name()
